      ...
      setError(
        translateError(
          caught instanceof Error
            ? caught.message
            : tr("Coach couldn't build that plan right now.")
        )
      )
    }
  }
      ...
      <div className="px-4 pt-4 sm:px-5 sm:pt-5">
        <div className="flex items-center gap-3">
          <h2 className="min-w-0 flex-1 truncate text-[18px] leading-tight font-semibold tracking-tight">
            {target?.exerciseName ?? tr("Ask Coach")}
          </h2>
          <button
            type="button"
              ...
              <div className="mt-4 border-t border-border/50 pt-3">
                <div className="flex items-baseline justify-between gap-3">
                  <p className="truncate text-[14px] font-bold">
                    {proposal.draft.name || tr("Today's plan")}
                  </p>
                  <span className="shrink-0 text-[11px] font-semibold text-muted-foreground">
                    <Message
                      text={"{{value0}} exercises"}
                      values={{ value0: proposal.draft.exercises?.length ?? 0 }}
                    />
                  </span>
                </div>
                  ...
                  {(proposal.draft.exercises ?? []).map((exercise, index) => (
                    ...
                    >
                      ...
                      <span className="shrink-0 text-[12px] text-muted-foreground">
                        <Message
                          text={"{{value0}} sets"}
                          values={{ value0: exercise.sets?.length || 3 }}
                        />
                      </span>
                    </div>
                  ))}
            ...
            <div className="rounded-[18px] border border-border/65 bg-card/25 p-2 transition-colors focus-within:border-foreground/30">
              <textarea
                ...
                onChange={(event) => {
                  ...
                  if (error) setError(translateError(""))
                }}
                onKeyDown={(event) => {
                ...
                placeholder={
                  target?.exerciseName
                    ? tr("What should I do instead of {{value0}}?", {
                        value0: target.exerciseName,
                      })
...
export function formatRetroDateLabel(date: string) {
  ...
  if (date === yesterday.toISOString().slice(0, 10)) return tr("Yesterday")
      ...
      <div className="px-6 pt-2 pb-6">
        ...
        <div className="flex items-baseline justify-between gap-3">
          ...
          <span
            ...
            aria-label={tr("Weights are read as {{value0}}", {
              value0: choice(unit === "lbs" ? "pounds" : "kilograms"),
            })}
          >
            {unit === "lbs" ? tr("lb") : tr("kg")}
          </span>
        </div>
        <textarea
          value={text}
    ...
    <MobileSheet onClose={onCancel} ariaLabel={tr("Save this workout")}>
      <div className="px-6 pt-2 pb-6">
        ...
        <p className="mt-1.5 text-[13px] leading-relaxed text-muted-foreground/80">
          <Message
            text={
              "{{value0}} of {{value1}} set{{value2}} marked done. Only completed sets are saved."
            }
            values={{
              value0: doneSets,
              value1: totalSets,
              value2: totalSets === 1 ? "" : "s",
            }}
          />
        </p>
    ...
    >
        ...
        <p className="mt-1.5 text-[13px] leading-relaxed text-muted-foreground/70">
          <Message
            ...
            values={{
              value0:
                !allDone &&
                tr("{{value0}} set{{value1}} still incomplete. ", {
                  value0: totalSets - doneSets,
                  value1: totalSets - doneSets > 1 ? "s" : "",
                }),
              value1: (
      ...
      <div className="flex flex-col gap-2 px-6 pt-4">
        <button
          ...
          className="h-[52px] w-full rounded-[20px] bg-foreground text-[15px] font-semibold tracking-tight text-background transition-opacity active:opacity-80 disabled:opacity-60"
        >
          {finishing ? tr("Finishing...") : tr("Finish workout")}
        </button>
        <button
          type="button"
          onClick={onCancel}
