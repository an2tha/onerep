...
export function coachErrorMessage(error: unknown): string {
  if (error && typeof error === "object" && "data" in error && typeof error.data === "string" && error.data.trim()) return error.data
  if (error instanceof Error && !error.message.includes("[CONVEX ")) return error.message
  return "Coach couldn’t answer right now. Please try again in a moment."
}
