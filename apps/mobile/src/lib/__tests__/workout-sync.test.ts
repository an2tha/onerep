...
describe("normalizePresetCard", () => {
  ...
  describe("focus normalization", () => {
    test("passes through 'strength' focus", () => {
      expect(normalizePresetCard({ ...base, focus: "strength" }).focus).toBe(
        "strength"
      )
    })

    test("passes through 'cardio' focus", () => {
      expect(normalizePresetCard({ ...base, focus: "cardio" }).focus).toBe(
        "cardio"
      )
    })

    test("passes through 'mobility' focus", () => {
      expect(normalizePresetCard({ ...base, focus: "mobility" }).focus).toBe(
        "mobility"
      )
    })

    test("unknown focus falls back to 'strength'", () => {
      expect(normalizePresetCard({ ...base, focus: "yoga" }).focus).toBe(
        "strength"
      )
    })

    test("null focus falls back to 'strength'", () => {
      expect(normalizePresetCard({ ...base, focus: null }).focus).toBe(
        "strength"
      )
    })
  ...
  describe("duration normalization", () => {
    test("uses provided duration", () => {
      expect(
        normalizePresetCard({ ...base, duration: "60 min" }).duration
      ).toBe("60 min")
    })

    test("null duration falls back to '30 min'", () => {
      expect(normalizePresetCard({ ...base, duration: null }).duration).toBe(
        "30 min"
      )
    })
  ...
  describe("steps normalization", () => {
    ...
    test("null steps falls back to default", () => {
      expect(normalizePresetCard({ ...base, steps: null }).steps).toEqual([
        "Warm up 5 min",
      ])
    })
    ...
    test("empty steps array falls back to default", () => {
      expect(normalizePresetCard({ ...base, steps: [] }).steps).toEqual([
        "Warm up 5 min",
      ])
    })
    ...
    test("single step is kept as-is", () => {
      expect(
        normalizePresetCard({ ...base, steps: ["Only step"] }).steps
      ).toEqual(["Only step"])
    })
  })
  ...
  describe("id and name passthrough", () => {
    ...
    test("preserves name", () => {
      expect(normalizePresetCard({ ...base, name: "Leg Day" }).name).toBe(
        "Leg Day"
      )
    })
  })
