...
router.get("/foods/search", async (req: Request, res: Response) => {
  ...
  const { q = "", limit = 25 } = validation.data;
...
router.get("/foods/id/:id", strictLimiter, async (req: Request, res: Response) => {
  ...
  if (!validation.success) return res.status(400).json({ error: validation.error.errors[0].message });
  const id = parseInt(validation.data.id);
...
router.get("/exercises/search", async (req: Request, res: Response) => {
  const validation = searchQuerySchema.safeParse(req.query);
  if (!validation.success) return res.status(400).json({ error: validation.error.errors[0].message });
  const { q = "", limit: size = 25 } = validation.data;
...
router.get("/exercises/lookup", async (req: Request, res: Response) => {
  ...
  if (!validation.success) return res.status(400).json({ error: "ids query param required" });
...
router.get("/exercises/advanced", async (req: Request, res: Response) => {
  const validation = searchQuerySchema.safeParse(req.query);
  if (!validation.success) return res.status(400).json({ error: validation.error.errors[0].message });
  const { muscle, equipment, category, force, limit = 100 } = validation.data;
