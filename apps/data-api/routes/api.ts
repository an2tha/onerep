...
router.get("/foods/nutrients", async (req: Request, res: Response) => {
  ...
  try {
    let results;
    if (grade) {
      results = await query("SELECT * FROM foodfacts WHERE nutriscore_grade = $1 LIMIT 100", [
        grade.toUpperCase(),
      ]);
    } else {
...
router.get("/foods/barcode/:code", strictLimiter, async (req: Request, res: Response) => {
  const validation = barcodeSchema.safeParse({ code: req.params.code });
  if (!validation.success) return res.status(400).json({ error: validation.error });
...
router.get("/exercises/search", async (req: Request, res: Response) => {
  const validation = searchQuerySchema.safeParse(req.query);
  if (!validation.success) return res.status(400).json({ error: validation.error });
  const { q = "", limit: size } = validation.data;
...
router.get("/exercises/advanced", async (req: Request, res: Response) => {
  const validation = searchQuerySchema.safeParse(req.query);
  if (!validation.success) return res.status(400).json({ error: validation.error });
  const { muscle, equipment, category, force, limit } = validation.data;
  ...
  try {
    const results = await query(`SELECT * FROM exercises ${where} LIMIT $${i}`, [
      ...params,
      limit,
    ]);
    res.json(results);
